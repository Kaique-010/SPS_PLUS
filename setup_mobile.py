...
from sps_plus import settings as django_settings


"""
Setup de ambiente mobile: agora usa as configurações ATUAIS do Django
(DATABASES['default']) e roda as migrações reais para criar as tabelas
essenciais (como django_session), evitando erros de "relation does not exist".

Se precisar executar o SQL customizado das tabelas/views específicas,
ele permanece disponível nas funções abaixo, mas por padrão está desativado.
"""

# Conexão usando o banco default do Django (conforme .env já carregado em settings)
DEFAULT_DB = django_settings.DATABASES.get('default', {})
LOCAL_DB_NAME = DEFAULT_DB.get('NAME') or config('LOCAL_DB_NAME', default=None)
LOCAL_DB_USER = DEFAULT_DB.get('USER') or config('LOCAL_DB_USER', default=None)
LOCAL_DB_PASSWORD = DEFAULT_DB.get('PASSWORD') or config('LOCAL_DB_PASSWORD', default=None)
LOCAL_DB_HOST = DEFAULT_DB.get('HOST') or config('LOCAL_DB_HOST', default='base.rtalmeida.com.br')
LOCAL_DB_PORT = DEFAULT_DB.get('PORT') or config('LOCAL_DB_PORT', default='5432')


# SQL para criar tabelas essenciais do Django
SQL_DJANGO_CORE = None  # Migrações reais substituem a criação manual das tabelas Django
...
def main():
    # Inicializa Django para usar settings e migrations reais
    os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'sps_plus.settings')
    django.setup()

    # 1) Migrações essenciais (reais) para apps do core
    print("📦 Rodando migrações essenciais reais (contenttypes, auth, sessions, admin)...")
    for app in ['contenttypes', 'auth', 'sessions', 'admin']:
        call_command('migrate', app, database='default', interactive=False, verbosity=1)

    # 2) Marcar como fake os apps que dependem de schema já existente (não gerenciados)
    print("🧩 Marcando migrações como FAKE para apps não gerenciados...")
    unmanaged_apps = [
        'licencas',
        'Agricola',
        'Entidades',
    ...
    for app in unmanaged_apps:
        try:
            call_command('migrate', app, database='default', fake=True, interactive=False, verbosity=1)
        except Exception as e:
            print(f"⚠️ Não foi possível marcar migrações de {app} como fake: {e}")

    # Opcional: executar SQL customizado somente se variável estiver ligada
    if os.getenv('RUN_MOBILE_SQL', 'False') == 'True':
        print("📦 Executando SQL customizado (RUN_MOBILE_SQL=True)...")
    ...
    print("🎉 Setup concluído: core migrado de verdade e demais apps marcados como fake.")
